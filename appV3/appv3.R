  ...
  sidebarLayout(position="left",
    ...
    mainPanel(
      tabsetPanel(
        ...
        tabPanel(title= "The DataSet",  DT::dataTableOutput("dta")),
...
server<-function(input, output){
  ...
  #0-reactive dta on selected DB
        dta<-reactive({ dta<-read.csv(input$selected_DB$datapath) }) 
                                  #reactive ++ to read it each time it changed (inout$selected_DB) and store it reactively
                                  #we read the path "file.csv"
        
        output$dta<-renderDataTable({ dta() })
    
  #1-Plotting function---- 
    #1-1-Reading plot arguments ----
        #Reading reactive plot arguments (dta and its variables(axes, color, regions, and countries))
        ...
        output$color<-renderUI({
          selectInput(inputId = "selected_color", label=p("Choose a color"), names(dta()) , selected=NULL)})

        output$filter_var<-renderUI({
          selectInput(inputId = "selected_filter", label="Chose the variable to filter on", names(dta()))})
        
        #reactive filter_choices on selected filter var:
        Filter<- reactive({Filter<-as.list(unique(dta()[input$selected_filter]))})# can't use unique(dta()$input$selected_filter, because input$selected_filter is a character
     ...
     #1-2-Making the Plot----
      output$reactiveplot<-renderPlotly ({
        ...
        #creating the plot
          ...
          #creating geoms
          L_geom<-L_dta
          for (i in 1: length(L_geom)){L_geom[[i]]<-geom(data=L_dta[[i]],mapping=aes(x=!!x, y=!!L_y[[i]],color=!!color), linetype=i , size=1/4, arrow=arrow())}
  ...
  #2- Summary stats function
    #Reading-reactively- Pivot arguments
        ...
        output$rows<-renderUI({selectInput("selected_rows", label="choose the rows to tabulate", names( dta() ), multiple = T )})
        output$columns<-renderUI({selectInput("selected_columns", label="choose the rows to tabulate", names( dta() ), multiple = T )})
        output$pivot_var<-renderUI({selectInput("selected_pivot_var", label="choose the vars to be considered in calculations", names( dta() ) )})
